...
fun WfaRequestResultScreen(
    ...
    Column(
        modifier = modifier.fillMaxSize().background(InfiniteColors.AttendanceReportBackground).padding(InfiniteSpacing.Default.xl),
        horizontalAlignment = Alignment.CenterHorizontally,
        verticalArrangement = Arrangement.Center
    ) {
        val result = uiState.submitResult
        if (result != null) {
            InfiniteCard(modifier = Modifier.fillMaxWidth().testTag("wfaSuccessResult"), semantic = InfiniteSemantic.Success) {
            ...
            val copy = WfaRequestUiMapper.map(uiState.failure ?: com.example.infinite_track.domain.model.booking.WfaRequestFailure.Unknown)
            InfiniteCard(modifier = Modifier.fillMaxWidth().testTag("wfaFailureResult"), semantic = InfiniteSemantic.Error) {
