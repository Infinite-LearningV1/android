...
class LoginViewModelReauthTest {
    ...
    fun `maps all reauth reasons to user facing banner messages`() = runTest {
        ...
        try {
            ...
            val expectations = mapOf(
                SessionManager.ReauthReason.INACTIVITY_EXPIRED to "Sesi tidak aktif lebih dari 48 jam. Silakan login lagi.",
                SessionManager.ReauthReason.REFRESH_INVALID to "Sesi tidak valid lagi. Silakan login lagi.",
                SessionManager.ReauthReason.REFRESH_REVOKED to "Sesi sudah berakhir. Silakan login lagi.",
                SessionManager.ReauthReason.NETWORK_OFFLINE_AT_REFRESH to "Tidak dapat memvalidasi sesi karena jaringan. Coba lagi setelah online."
            )
    ...
    fun `dismiss reauth banner clears session manager state`() = runTest {
        ...
        try {
            val sessionManager = SessionManager()
            val viewModel = createViewModel(sessionManager)
            sessionManager.triggerForcedReauth(SessionManager.ReauthReason.REFRESH_INVALID)
            advanceUntilIdle()
